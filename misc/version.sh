...
#    1.2.3-rc1  v1.2.3-rc1-gita1b2c3d4          2001-01-01 00:00:00 +0000		# release
#    1.2.3-rc1  v1.2.3-rc1-789-ga1b2c3d4-dirty  2001-01-01 00:00:00 +0000		# unreleased (with .git/)
#    1.2.3-rc1  v1.2.3-rc1-unreleased-ga1b2c3d4 2001-01-01 00:00:00 +0000		# unreleased (via git-archive)
...
if test "$GITARCHIVE_HASH" = "${GITARCHIVE_HASH/:/}" ; then
  ...
  if test -n "$GITARCHIVE_TAG" -a "${GITARCHIVE_TAG#v}" = "$NEWS_VERSION" ; then
    ...
    exit_with_version "$NEWS_VERSION" "v$NEWS_VERSION-git${GITARCHIVE_HASH:0:13}" "$GITARCHIVE_DATE"
  else
    # not a release (git-archive)
    exit_with_version "$NEWS_VERSION" "v$NEWS_VERSION-unreleased-g${GITARCHIVE_HASH:0:13}" "$GITARCHIVE_DATE"
  fi
fi
...
test -n "$COMMIT_DATE" && {
  NEWS_VERSION="$(peek_news_version)"
  BUILD_ID="v$NEWS_VERSION-unreleased-g`shorthash`"
