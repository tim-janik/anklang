...
/// Test that a window resize triggers a debounced Shell relayout (M6).
/// After the SolidJS migration a resize must re-mount the Shell tree so that
/// components which compute pixel sizes once at mount time (level meters, clip
/// canvases) recompute them. The relayout is debounced (restart=true,
/// wait=500ms): repeated resizes within the window must collapse into a single
/// relayout, and the relayout must re-mount the DOM (old .b-shell detached).
async function test_relayout_resize (): Promise<boolean>
{
  ...
  // Drain the boot-time debounced relayout scheduled by dpr_rerender_all() so
  // it cannot fire during the measured window below.
...
/// Test that a relayout preserves the current interactive state.
/// App.relayout re-mounts the Shell tree but must keep the selected track
/// (Shell.reset() would otherwise reset it to the initial master track).
...
/// Test that the relayout is a no-op before any project is assigned.
/// The boot-time dpr_rerender_all() call happens before App.assign_project(), so
/// the resize/DPR handler must guard against App.project being null.
async function test_relayout_guard_no_project (): Promise<boolean>
{
  ...
  // Temporarily hide the project by nulling Shell.project; App.project is
  // `globalThis.Shell?.project ?? null`, simulating the pre-boot state without
  // tearing down the actual Shell.
